...
function comparisonMarkup(routes) {
  const rows = [
    ['ルートの特徴', route => route.kindSummary],
    ['この進路で扱うこと', route => route.why],
    ['必要な数学', route => route.math.label],
    ['高校までに決めること', route => route.steps.find(step => step.stage === 'course').title],
  ...
  return `<div class="route-table-scroll" tabindex="0" role="region" aria-label="進路ルートの違いを比べる表。横にスクロールできます">
...
function timelineMarkup() {
  return `<section class="deferral-board" aria-label="進路を選ぶ時期の目安">
    <h2>この先の選択は、いつまでに決めればよいか</h2>
    <p class="deferral-lead">今日決める必要があるのは、現在に近い1つだけです。その先の3つは、まだ決める時期ではありません。</p>
...
export function renderRoutes({domainId, domain, saved, checkedOn}) {
  ...
  return `<section class="route-intro">
      <p class="eyebrow">${escape(domain.name)} までの進路</p>
      <h2>${escape(domain.name)}へ進むルートは、${routes.length}通りあります。</h2>
      <p>それぞれ、高校までに決める内容が違います。どれかが正解ということはありません。</p>
