...
`version: 2`（Build 19 で `verbs`・`domains` を追加）。実行可能な検証仕様は `wireframe/scripts/knowledge.mjs`。追加前後に `knowledge:check`、`knowledge:build`、`check`、`test`、`build` を実行する。新しい外部依存は不要。

## 混ぜないもの

| データ | 意味 | 例 |
|---|---|---|
| verbs | 逆算の起点になる動詞 | つくる、くらべる、観察する |
| domains | 学びの領域 | 生態学、情報科学 |
| topics | 興味の入口 | 音楽、料理 |
...
```json
{
  ...
  "domains": ["media", "design", "information", "engineering", "food", "sound"]
}
```

検証で強制していること：

- 動詞は3〜8個。増やすと分類作業になり、中学生が持っていない語彙に戻る。
- 各動詞に活動が3件以上。足りない動詞は入口として成立しないので、公開させない。
- 各動詞が2つ以上の領域に着地する（`core.test.mjs`）。1領域しか持たない動詞は、領域の言い換えにすぎない。
- 各動詞が2つ以上の興味をまたぐ（`core.test.mjs`）。ゲームの「つくる」と料理の「つくる」が同じ場所に出ることが、この層の目的である。

## domains（Build 19 で追加）

これまで `app.js` と `catalog.mjs` に直接書かれていた8領域を、データへ移した。画面のコードに領域名を書かない。

## activities に足した項目

| 項目 | 意味 |
|---|---|
| `verb` | どの動詞から到達するか。全件必須 |
| `minutes` | 所要時間の目安（5〜180）。「今日できること」の判断に使う |
| `athome` | 家だけで完結するか。人や場所が要るものと区別する |

活動の文言は Build 17 の編集をそのまま使っている。動詞・時間・場所の割り当てがこのアプリの編集であることを、画面に明記する。

## resources に足した項目

`verbs`（配列）。学校・大学（`group: "study"`）は動詞で選ぶものではないので空を許すが、それ以外は1つ以上必須。動詞から到達できない掲載情報を作らないための制約。
